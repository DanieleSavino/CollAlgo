...
CB_Error_t CA_bench_bine_reduce_cuda(void) {
    ...
    CA_CUDA_MALLOC_LABEL(send_buff, BUFF_SIZE * sizeof(int), cleanup);
    CA_bench_reduce_fill_cuda(send_buff, rank, BUFF_SIZE);

    if (rank == ROOT)
        CA_CUDA_MALLOC_LABEL(recv_buff, BUFF_SIZE * sizeof(int), cleanup);

    CA_bine_reduce(send_buff, recv_buff, BUFF_SIZE, MPI_INT, MPI_SUM, ROOT, MPI_COMM_WORLD);

    if (rank == ROOT) {
        CA_CUDA_MALLOC_LABEL(d_errors, sizeof(int), cleanup);
        CA_CUDA_CHECK_LABEL(cudaMemset(d_errors, 0, sizeof(int)), cleanup);
        CA_bench_reduce_check_cuda(recv_buff, size, BUFF_SIZE, d_errors);
        int h_errors = 0;
        CA_CUDA_CHECK_LABEL(cudaMemcpy(&h_errors, d_errors, sizeof(int),
